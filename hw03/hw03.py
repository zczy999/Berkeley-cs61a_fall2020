...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        lefts = totals_tree(end(left(m)))
        rights = totals_tree(end(right(m)))
        root = label(lefts) + label(rights)
        return tree(root,[lefts,rights])
